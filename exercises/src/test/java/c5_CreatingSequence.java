...
public class c5_CreatingSequence {
    ...
    public void generate_programmatically() {
        ...
        // This is for synchronous and one-by-one emissions, meaning that the sink is a SynchronousSink and that its
        // next() method can only be called at most once per callback invocation.
        Flux<Integer> generateFlux = Flux.generate(
                AtomicInteger::new,
                (state, sink) -> {
                    int number = state.getAndIncrement();
                    sink.next(number);
        ...
        // Asynchronous and multi-threaded. It exposes a FluxSink, with its `next`, `error` and `complete` methods.
        // contrary to `generate`, it doesn't have a state-based variant. On the other hand, it can trigger
        // multi-threaded events in the callback.
        ...
        // Asynchronous but single-threaded. It is a middle ground between generate and create which is suitable for
        // processing events from a single producer. It is similar to create in the sense that it can also be
        // asynchronous and can manage backpressure using any of the overflow strategies supported by create. However,
