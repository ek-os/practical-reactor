...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor_again() {
        // There are three dimensions that can be compared between concatMap, flatMap and flatMapSequential:
        //
        // concatMap:
        ...
        //     this operator naturally preserves the same order as the source elements, concatenating the inners from
        //     each source element sequentially.
        // * Interleaving:
        //     this operator does not let values from different inners interleave (concatenation).
        //
        // flatMap:
        // * Generation of inners and subscription:
        //     this operator is eagerly subscribing to its inners.
        // * Ordering of the flattened values:
        //     this operator does not necessarily preserve original ordering, as inner element are flattened as they
        //     arrive.
        // * Interleaving:
        //     this operator lets values from different inners interleave (similar to merging the inner sequences).
        //
        // flatMapSequential:
        // * Generation of inners and subscription:
        //     this operator is eagerly subscribing to its inners (like flatMap).
        // * Ordering of the flattened values:
        //     this operator queues elements from late inners until all elements from earlier inners have been emitted,
        //     thus emitting inner sequences as a whole, in an order that matches their source's order.
        // * Interleaving:
        //     this operator does not let values from different inners interleave (similar looking result to concatMap,
        //     but due to queueing of values that would have been interleaved otherwise).
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, flux) -> {
            if (signal.hasValue()) {
                var msg = signal.get();
                return msg.metaData.equals("spam")
                        ? mailBoxSecondary()
                        : flux;
            }
            return flux;
